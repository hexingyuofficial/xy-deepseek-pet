...
| `xy-deepseek-desktop` | Electron runtime, schema, default whale theme | Pulled by `xy-deepseek-pet` |
...
The main plugin depends on the exact matching `xy-deepseek-desktop` version. Its small CLI resolves the package-local Electron executable, then starts `dist/main.js`. The desktop tarball contains:
