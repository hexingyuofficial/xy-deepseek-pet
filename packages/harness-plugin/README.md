...
不能在旧服务仍占用端口时再启动一份。首次安装会下载约 120-150 MB 的 Electron 平台运行时，下载阶段可能暂时没有进度输出。重启后从 Harness 侧边栏打开桌宠，所有偏好都在“通用设置 > 桌面宠物”。支持真实任务状态、最多三个会话气泡、精确会话回复、气泡内审批、20%-200% 缩放、原生/Petdex ZIP 主题导入、可选桌面快捷方式和数据化右键菜单扩展。
